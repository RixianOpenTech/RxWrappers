...
namespace System.IO
{
    public static class __StringReader
    {
        public static IObservable<System.Reactive.Unit> Close(this IObservable<System.IO.StringReader> StringReaderValue)
        {
            return
                Observable.Do(StringReaderValue, (StringReaderValueLambda) => StringReaderValueLambda.Close()).ToUnit();
        }
        ...
        public static IObservable<System.String> ReadLineAsync(
            this IObservable<System.IO.StringReader> StringReaderValue)
        {
            return
                Observable.Select(StringReaderValue,
                    (StringReaderValueLambda) => StringReaderValueLambda.ReadLineAsync().ToObservable()).Flatten();
        }
            ...
            this IObservable<System.IO.StringReader> StringReaderValue)
        {
            return
                Observable.Select(StringReaderValue,
                    (StringReaderValueLambda) => StringReaderValueLambda.ReadToEndAsync().ToObservable()).Flatten();
        }


        public static IObservable<System.Int32> ReadBlockAsync(
            this IObservable<System.IO.StringReader> StringReaderValue, IObservable<System.Char[]> buffer,
            IObservable<System.Int32> index, IObservable<System.Int32> count)
        {
            return
                Observable.Zip(StringReaderValue, buffer, index, count,
                    (StringReaderValueLambda, bufferLambda, indexLambda, countLambda) =>
                        StringReaderValueLambda.ReadBlockAsync(bufferLambda, indexLambda, countLambda).ToObservable())
                    .Flatten();
        }


        public static IObservable<System.Int32> ReadAsync(this IObservable<System.IO.StringReader> StringReaderValue,
            IObservable<System.Char[]> buffer, IObservable<System.Int32> index, IObservable<System.Int32> count)
        {
            return
                Observable.Zip(StringReaderValue, buffer, index, count,
                    (StringReaderValueLambda, bufferLambda, indexLambda, countLambda) =>
                        StringReaderValueLambda.ReadAsync(bufferLambda, indexLambda, countLambda).ToObservable())
                    .Flatten();
        }
    }
}